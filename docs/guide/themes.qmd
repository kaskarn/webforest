...
webforest includes 9 preset themes designed for different use cases. All themes can be customized using a fluent API.
...
```r
set_container(
  border = FALSE,        # Show border around container (default: off)
  border_radius = 8      # Corner radius in pixels
)
```
...
| Layout | `set_layout()` | plot_position, row_border, row_border_style |
| Container | `set_container()` | border, border_radius |
